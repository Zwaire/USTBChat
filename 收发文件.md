...
你在 `handle_client()` 的消息类型判断里，直接加下面两个 `elif`。
...
# 六、如果你不想额外拆函数，最简版可直接塞进 `handle_client()`

如果你队友现在只想“赶紧跑起来”，那你甚至可以不用上面那些函数，直接把下面代码插到 `handle_client()` 里。
